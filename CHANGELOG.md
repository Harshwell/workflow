...
- Routing MAIN/SUB memindahkan Sitcomtara ke `SC - Meilani`, mengarahkan seluruh EzCare/Ez Care dengan `Device Brand` Apple ke `SC - Farhan`, membatasi lima replacement-delivery status hanya ke `Finish`, dan memmirror row `Start` berdasarkan dropdown manual `Status = Delivered` (bukan `Last Status`) dengan prioritas PIC/Branch lalu fallback Service Center; mirror mengisi SC `Type = Start`, tidak membawa lima field khusus Start (`AWB`, `Timestamp AWB`, `Branch`, `Claim Type`, `Service Center PIC`), dan tetap dipertahankan saat status manual berikutnya berubah.
