 ...
 * Source : Raw Data
 * Filter :
 * - device_brand equals "Samsung"
 * - claim_submitted_datetime >= 2026-06-01
...
const SAMSUNG_CLAIM_SYNC_CONFIG = {
  ...
  TARGET_SPREADSHEET_ID: '1eXrN6pFXHr1-Qj208tzsMa5ysXgEv84LG-XiV6HhcJI',

  SOURCE_HEADER_ROW: 1,
  TARGET_HEADER_SCAN_ROWS: 20,
  TARGET_HEADER: 'Claim Number',
  RUN_TIMESTAMP_CELL: 'G1',
  RUN_STATUS_CELL: 'G2',
  ...
  MIN_SUBMITTED_DATE: new Date(2026, 5, 1), // 1 Jun 2026
  DAILY_TRIGGER_HOUR: 9
};

const SAMSUNG_CLAIM_EXCLUDED_STATUSES = new Set([
  'CLAIM_INITIATE',
  'QOALA_ASK_DETAIL',
...
function onOpen() {
  SpreadsheetApp.getUi()
    .createMenu('Samsung Claim')
    .addItem('Recheck Claim Number', 'manualRecheckSamsungClaims')
    .addSeparator()
    .addItem('Setup / Reset Automation', 'setupSamsungClaimSync')
    .addToUi();
}

function setupSamsungClaimSync() {
  createAutomationTriggers_();
  const result = syncSamsungClaims_('INITIAL_SETUP');

  showAlertSafe_(
    'Samsung Claim Sync Berhasil',
    buildResultMessage_(result) +
      '\n\nTrigger OnEdit REQ FU dan sync harian pukul 09:00 sudah aktif.'
  );
}

function manualRecheckSamsungClaims() {
  try {
    showToastSafe_('Sedang melakukan full recheck...', 'Samsung Claim', 5);

    const result = syncSamsungClaims_('MANUAL_RECHECK');
    ...
    showAlertSafe_(
      'Samsung Claim Recheck Selesai',
      buildResultMessage_(result)
    );
  } catch (error) {
    console.error(`[MANUAL_RECHECK] ERROR: ${error.message}`);
    showAlertSafe_('Samsung Claim Recheck Gagal', error.message);
    throw error;
  }
}

function dailySamsungClaimSync() {
  syncSamsungClaims_('DAILY_09');
}

function syncSamsungClaims_(mode) {
  ...
  try {
    ...
    console.log(`[${mode}] Mulai Samsung Claim Sync`);
    ...
    if (!sourceSheet) {
      throw new Error(`Sheet "${SAMSUNG_CLAIM_SYNC_CONFIG.SOURCE_SHEET_NAME}" tidak ditemukan.`);
    }
    ...
    if (lastRow === 0 || lastColumn === 0) {
      ...
      writeSamsungClaimRunStatus_(startedAt, 'UPDATED');
      return result;
    }

    const values = sourceSheet
      .getRange(
        SAMSUNG_CLAIM_SYNC_CONFIG.SOURCE_HEADER_ROW,
        1,
        lastRow - SAMSUNG_CLAIM_SYNC_CONFIG.SOURCE_HEADER_ROW + 1,
        lastColumn
      )
      .getValues();
    ...
    const claimIndex = findSourceHeaderIndex_(
      headers,
      SAMSUNG_CLAIM_SYNC_CONFIG.SOURCE_HEADERS.CLAIM_NUMBER
    );
    const brandIndex = findSourceHeaderIndex_(
      headers,
      SAMSUNG_CLAIM_SYNC_CONFIG.SOURCE_HEADERS.DEVICE_BRAND
    );
    const statusIndex = findSourceHeaderIndex_(
      headers,
      SAMSUNG_CLAIM_SYNC_CONFIG.SOURCE_HEADERS.STATUS
    );
    const submittedIndex = findSourceHeaderIndex_(
      headers,
      SAMSUNG_CLAIM_SYNC_CONFIG.SOURCE_HEADERS.SUBMITTED_AT
    );
    ...
    for (let i = 1; i < values.length; i++) {
      ...
      if (deviceBrand !== 'samsung') continue;
      samsungRows++;

      const submittedAt = parseSheetDate_(row[submittedIndex]);
      if (!submittedAt || submittedAt < SAMSUNG_CLAIM_SYNC_CONFIG.MIN_SUBMITTED_DATE) continue;
      dateEligibleRows++;

      const status = String(row[statusIndex] ?? '').trim().toUpperCase();
      if (!status || SAMSUNG_CLAIM_EXCLUDED_STATUSES.has(status)) continue;
    ...
    if (locked) writeSamsungClaimRunStatusSafe_(startedAt, 'FAILED', mode);
...
function writeTargetClaims_(claimNumbers) {
  ...
  const targetSheet = targetSS.getSheetByName(SAMSUNG_CLAIM_SYNC_CONFIG.TARGET_SHEET_NAME);

  if (!targetSheet) {
    throw new Error(
      `Sheet tujuan "${SAMSUNG_CLAIM_SYNC_CONFIG.TARGET_SHEET_NAME}" tidak ditemukan di target.`
    );
  }
    ...
    if (existingClaims.has(normalizedClaim)) {
      throw new Error(
        `Duplicate Claim Number "${normalizedClaim}" ditemukan di target ` +
          `sheet "${SAMSUNG_CLAIM_SYNC_CONFIG.TARGET_SHEET_NAME}" sekitar row ` +
...
function createAutomationTriggers_() {
  createSamsungClaimOnEditTrigger_(false);
  createDailyTrigger_();
}

function createDailyTrigger_() {
  const handlerName = 'dailySamsungClaimSync';

  deleteTriggersByHandler_(handlerName);

  ScriptApp.newTrigger(handlerName)
    .timeBased()
    .atHour(SAMSUNG_CLAIM_SYNC_CONFIG.DAILY_TRIGGER_HOUR)
    .everyDays(1)
    .create();

  console.log(
    `[TRIGGER] Daily trigger pukul ${String(SAMSUNG_CLAIM_SYNC_CONFIG.DAILY_TRIGGER_HOUR).padStart(2, '0')}:00 berhasil dibuat/reset.`
  );
}
  ...
  if (index === -1) {
    throw new Error(
      `Header source "${headerName}" tidak ditemukan pada row ` +
        `${SAMSUNG_CLAIM_SYNC_CONFIG.SOURCE_HEADER_ROW} sheet ` +
        `"${SAMSUNG_CLAIM_SYNC_CONFIG.SOURCE_SHEET_NAME}".`
    );
  }
  return index;
...
function findTargetClaimHeader_(targetSheet) {
  ...
  const scanRows = Math.min(
    Math.max(lastRow, 1),
    SAMSUNG_CLAIM_SYNC_CONFIG.TARGET_HEADER_SCAN_ROWS
  );
  if (lastColumn === 0) {
    throw new Error(
      `Sheet target "${SAMSUNG_CLAIM_SYNC_CONFIG.TARGET_SHEET_NAME}" kosong; ` +
        `header "${SAMSUNG_CLAIM_SYNC_CONFIG.TARGET_HEADER}" tidak ditemukan.`
    );
  }

  const values = targetSheet.getRange(1, 1, scanRows, lastColumn).getValues();
  const expected = normalizeTargetHeader_(SAMSUNG_CLAIM_SYNC_CONFIG.TARGET_HEADER);
  ...
  throw new Error(
    `Header target "${SAMSUNG_CLAIM_SYNC_CONFIG.TARGET_HEADER}" tidak ditemukan ` +
      `pada row 1-${scanRows} sheet "${SAMSUNG_CLAIM_SYNC_CONFIG.TARGET_SHEET_NAME}".`
  );
}

function writeSamsungClaimRunStatus_(startedAt, status) {
  const targetSS = SpreadsheetApp.openById(
    SAMSUNG_CLAIM_SYNC_CONFIG.TARGET_SPREADSHEET_ID
  );
  const targetSheet = targetSS.getSheetByName(
    SAMSUNG_CLAIM_SYNC_CONFIG.TARGET_SHEET_NAME
  );
  if (!targetSheet) {
    throw new Error(
      `Sheet tujuan "${SAMSUNG_CLAIM_SYNC_CONFIG.TARGET_SHEET_NAME}" tidak ditemukan di target.`
  ...
  const timestampRange = targetSheet.getRange(
    SAMSUNG_CLAIM_SYNC_CONFIG.RUN_TIMESTAMP_CELL
  );
  timestampRange
    .setValue(startedAt)
    .setNumberFormat(SAMSUNG_CLAIM_SYNC_CONFIG.RUN_TIMESTAMP_FORMAT);
  targetSheet
    .getRange(SAMSUNG_CLAIM_SYNC_CONFIG.RUN_STATUS_CELL)
    .setValue(status);
}

function writeSamsungClaimRunStatusSafe_(startedAt, status, mode) {
  try {
    writeSamsungClaimRunStatus_(startedAt, status);
  } catch (statusError) {
    console.error(
...
function buildResultMessage_(result) {
  return [
    ...
    `Samsung rows: ${result.samsungRows}`,
    `Date >= Jun 2026: ${result.dateEligibleRows}`,
...
function showToastSafe_(message, title, seconds) {
  try {
    ...
    spreadsheet.toast(message, title || 'Samsung Claim', seconds || 5);
  } catch (error) {
