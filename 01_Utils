 ...
 * Delegates to tolerant header matching so legacy helper names do not break
 * sorting/schema code paths.
 */
function findHeaderIndexByCandidates_(headerRowValues, candidates, opts) {
  return findHeaderIndexAny_(headerRowValues, candidates, opts);
}
...
/**
 * Allow native Date parsing only for strings that carry an explicit timezone.
 * This avoids silent locale/timezone drift for ambiguous date strings.
 */
function canUseNativeDateParse_(value) {
  const s = String(value == null ? '' : value).trim();
  if (!s) return false;
  return /(?:[zZ]|[+-]\d{2}:?\d{2})$/.test(s) || /GMT/i.test(s) || /UTC/i.test(s);
}

function tryNativeParseUnambiguousDate_(value) {
  if (!canUseNativeDateParse_(value)) return null;
...
// Numeric string: only treat as serial/epoch if plausible (avoid money->date)
  ...
  // Last resort: allow native parsing only for strings with explicit timezone information.
  const dNative = tryNativeParseUnambiguousDate_(s0);
 ...
 * ========================= */
function computeDbValueFromClaimNumber_(claimNumber) {
...
function parseClaimLastUpdatedDatetime_(v) {
  ...
  const m = s.match(/^\s*([A-Za-z]+)\s+(\d{1,2})\s*,\s*(\d{4})\s*,\s*(\d{1,2})\s*:\s*(\d{2})(?::(\d{2}))?\s*([AaPp][Mm])\s*$/);
  if (!m) return null;
  ...
  if (!month || isNaN(day) || isNaN(year) || isNaN(hh) || isNaN(mm) || isNaN(ss)) return null;

  if (ampm === 'PM' && hh < 12) hh += 12;
  if (ampm === 'AM' && hh === 12) hh = 0;

  const iso = Utilities.formatString('%04d-%02d-%02d %02d:%02d:%02d', year, month, day, hh, mm, ss);
  try {
...
function sortSheetDataByHeaderSpecs_(sheet, specs, headerRowIndex) {
  ...
  (specs || []).forEach(s => {
    ...
    const colIdx0 = findHeaderIndexByCandidates_(header, candidates, { headerIndex: idxMap, enableSnakeCase: true });
...
function assertSheetHasHeaders_(sheet, requiredHeaders, context) {
  ...
  const hdrCi = buildHeaderIndexCi_(hdr);
  ...
  for (let i = 0; i < req.length; i++) {
    ...
    const idx0 = (typeof findHeaderIndexAny_ === 'function')
      ? findHeaderIndexAny_(hdr, [h], { enableSnakeCase: true })
      : ((hdrCi[normalizeHeaderKey_(h)] != null) ? hdrCi[normalizeHeaderKey_(h)] : null);

    if (idx0 == null || idx0 < 0) missing.push(h);
  }
