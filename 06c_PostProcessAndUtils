...
function __toDate06_(v) {
  ...
  if (typeof v === 'number') {
    // Google Sheets serial dates are usually returned as Date objects; this is a best-effort fallback.
...
function getStatusTypeMap06c_() {
  // Allow override via CONFIG (centralized) if present.
  try {
    if (CONFIG && CONFIG.STATUS_TYPE_MAP) return CONFIG.STATUS_TYPE_MAP;
  } catch (e) {}

  // Fallback: mapping provided by user.
  return {
    'CLAIM_INITIATE': 'OPEN',
...
function parseClaimLastUpdatedDatetime06c_(v) {
  ...
  // STRICT parsing (WIB) to avoid server-timezone shifts.
  const patterns = [
    'MMMM d, yyyy, h:mm a',      // "February 16, 2026, 5:49 PM"
    'MMMM d, yyyy, h:mm:ss a',   // with seconds
    'MMM d, yyyy, h:mm a',       // abbreviated month
    'yyyy-MM-dd HH:mm:ss',
    'yyyy-MM-dd HH:mm',
    'dd/MM/yyyy HH:mm',
    'd/M/yyyy H:mm'
  ...
  const iso = year + '-' + pad2(month) + '-' + pad2(day) + ' ' + pad2(hh) + ':' + pad2(mm) + ':' + pad2(ss);
  try {
...
function sortOperationalSheetPreserveFilter06c_(sheet) {
  ...
  try {
    ...
    if (filter && filter.getRange) {
      ...
      if (nr > 1) {
        // Sort body only (exclude header), keep filter intact.
        sheet.getRange(sr + 1, sc, nr - 1, nc).sort(sortSpec);
      }
      return;
    }
  } catch (e0) {}
  ...
  try {
    sheet.getRange(1, 1, lr, lc).sort(sortSpec);
  } catch (e1) {}
}
...
function __loadExistingEventIds06c_(webappSs, dailyName, pastName) {
  ...
  function loadOne(name) {
    ...
    const n = lr - 1;
    const vals = sh.getRange(2, idxEvent + 1, n, 1).getValues();
    for (let i = 0; i < n; i++) {
