...
| --- | --- | --- | --- |
| Apple Claim Sync | `setupAppleClaimSync()` memasang ulang installable OnEdit dan time-driven trigger harian pukul 09:00 sesuai timezone Apps Script; `dailyAppleClaimSync()` menjalankan scheduled sync, sedangkan menu menyediakan full recheck manual. | Script lock men-serialize OnEdit, scheduled, setup, dan manual sync; installer menghapus trigger dengan handler sama sebelum membuat pengganti agar tidak terduplikasi. | Setiap run membaca `Raw Data`, memfilter claim Apple eligible, melakukan dedupe `Claim Number`, lalu mengganti managed target column secara batch. Jalankan setup sekali dari editor dan set timezone project dengan benar; kegagalan scheduled run tercatat di Apps Script Executions dan source/target tidak dibersihkan sebelum pembacaan serta validasi source berhasil. |
