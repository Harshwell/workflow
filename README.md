...
| `optional-project/SC-Meilani.js` | Manual mirror Salvage dan upsert Salvage Repair milik Meilani. | Branch/status/header contract project ini berubah. |
...
| --- | --- | --- | --- |
| Service Center Extractor | `runServiceCenterTransfer()`; menu via `onOpen()`. | Script lock, minimum run interval, RunID properties. | Batch read/write, append log `Log - SC Transfer`, unmapped quarantine, verify destination; project punya config/deployment sendiri. |
| SC-Meilani | Menu `Salvage`, `Salvage Repair`, `Run All`. | Script lock, active-run/stop properties. | Mirror/upsert by claim, structured step logs, post-write verification, cooperative stop. |
