...
| --- | --- | --- | --- |
| Apple Claim Sync | `setupAppleClaimSync()` memasang ulang installable OnEdit untuk workbook source dan target serta time-driven trigger harian pukul 09:00 sesuai timezone Apps Script; `dailyAppleClaimSync()` menjalankan scheduled sync, sedangkan menu menyediakan full recheck manual. OnEdit pada target `REQ FU`.`Status` (kolom N) memasang strikethrough dan fill abu-abu pada seluruh baris untuk nilai `CLOSED`, lalu menghapus kedua format tersebut ketika status berubah. | Script lock men-serialize OnEdit sync `Raw Data`, scheduled, setup, dan manual sync; formatting `REQ FU` hanya memproses baris yang diedit. Installer menghapus semua trigger dengan handler sama sebelum membuat pengganti agar tidak terduplikasi. | Setiap sync membaca `Raw Data`, memfilter claim Apple eligible, melakukan dedupe `Claim Number`, lalu mengganti managed column `Claim Number` pada target sheet bernama `REQ FU` secara batch—bukan memilih tab berdasarkan GID. Jalankan setup sekali dari editor dan set timezone project dengan benar; kegagalan scheduled run tercatat di Apps Script Executions dan target tidak dibersihkan sebelum sheet serta header source/target tervalidasi. |
