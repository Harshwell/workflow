...
function getUnmappedPartnerMinSubmissionDate06_() {
  // Prefer config, fallback to 2025-06-01.
  let s = null;
  try {
    s = CONFIG && CONFIG.DETAILS_LOG_POLICY && CONFIG.DETAILS_LOG_POLICY.UNMAPPED_PARTNER_MIN_SUBMISSION_DATE
      ? CONFIG.DETAILS_LOG_POLICY.UNMAPPED_PARTNER_MIN_SUBMISSION_DATE
      : null;
  } catch (e) {}
  ...
  const d = __toDate06_(s);
  return d ? new Date(d.getFullYear(), d.getMonth(), d.getDate()) : fallback;
}
...
try {
  ...
  try { logLine_('WEBAPP_SNAP_PREV_ERR', 'Snapshot PREV failed (non-fatal)', String(eWp0), '', 'WARN'); } catch (eWp2) {}
    ...
    const queueLabel = String(pMerged.QUEUED_LABEL || pMerged.QUEUE_LABEL || 'QUEUED_SUB').trim();
...
function __appendSubmissionFromRawIfMissing06a_(ss, rawMap, rawHdrIdx, dbTag) {
  ...
  let wantStatus = 'CLAIM_INITIATE'; // Strict: only CLAIM_INITIATE can be inserted as NEW claim (Submission)

  // Prefer configured DB tag mapping when available, but keep strict insert rule.
  try {
    if (flow && flow.SUBMISSION_RULES) {
      if (dbValue === 'OLD' && flow.SUBMISSION_RULES.OLD && flow.SUBMISSION_RULES.OLD.DB_VALUE) {
        dbValue = String(flow.SUBMISSION_RULES.OLD.DB_VALUE || '').trim().toUpperCase();
      } else if (dbValue === 'NEW' && flow.SUBMISSION_RULES.NEW && flow.SUBMISSION_RULES.NEW.DB_VALUE) {
        dbValue = String(flow.SUBMISSION_RULES.NEW.DB_VALUE || '').trim().toUpperCase();
      }
    }
  } catch (e0) {}

  // Enforce strict rule regardless of config
  wantStatus = 'CLAIM_INITIATE';
