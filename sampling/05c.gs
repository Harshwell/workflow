...
function __isDryRun05c__() {
  try { return !!DRY_RUN; } catch (e) { return false; }
}
...
function __parseClaimLastUpdatedDatetime05c_(v) {
  ...
  // Fallback parse for strings like: "January 24, 2025, 6:06 PM"
  try {
...
function processB2B_(ss, rawValues, headerIndexRaw, pic) { // `pic` kept for backward compatibility
  ...
  const EXCLUDED_LAST_STATUSES = (typeof __EXCLUDED_LAST_STATUSES !== 'undefined' && __EXCLUDED_LAST_STATUSES && __EXCLUDED_LAST_STATUSES.size)
    ? __EXCLUDED_LAST_STATUSES
  ...
  let headerRow = sh.getRange(1, 1, 1, sh.getLastColumn()).getValues()[0].map(__normalizeHeaderText05c_);
...
function processSpecialCase_(ss, rawValues, headerIndexRaw, pic) { // `pic` kept for backward compatibility
  ...
  let header = sh.getRange(1, 1, 1, sh.getLastColumn()).getValues()[0].map(__normalizeHeaderText05c_);
  let idxH = buildHeaderIndex_(header);
...
function processEVBike_(ss, rawValues, headerIndexRaw, pic) { // `pic` kept for backward compatibility
  ...
  try {
    ...
    if (subSh && subSh.getLastRow() > 1 && subSh.getLastColumn() > 1) {
      const subHeader = subSh.getRange(1, 1, 1, subSh.getLastColumn()).getValues()[0].map(__normalizeHeaderText05c_);
