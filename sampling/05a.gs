...
var __EXCLUDED_LAST_STATUSES = (function () {
  // Prefer per-run builder from 00_Config if available
  try { if (typeof getExcludedLastStatusesSet_ === 'function') return getExcludedLastStatusesSet_(); } catch (e) {}
  try { if (typeof buildExcludedLastStatusesSet_ === 'function') return buildExcludedLastStatusesSet_(); } catch (e) {}

  // Back-compat: older versions exposed a Set directly
  try {
    if (typeof EXCLUDED_LAST_STATUSES !== 'undefined' &&
        EXCLUDED_LAST_STATUSES &&
        EXCLUDED_LAST_STATUSES.has) return EXCLUDED_LAST_STATUSES;
  } catch (e) {}

  // Newer versions may expose a base array
  try {
    if (typeof EXCLUDED_LAST_STATUSES_BASE !== 'undefined' &&
        Array.isArray(EXCLUDED_LAST_STATUSES_BASE)) return new Set(EXCLUDED_LAST_STATUSES_BASE);
  } catch (e) {}
...
function mutateRawInMemory_(rawValues, headerIndexRaw, agingStdMap, agingMap, assocMap) {
  ...
  const idxClaim = idxAny_(headerIndexRaw, [h.claimNumber, 'Claim Number', 'claim_number']);
  const idxBP = idxAny_(headerIndexRaw, [h.businessPartner, 'Business Partner', 'Partner', 'business_partner']);
  const idxAssoc = idxAny_(headerIndexRaw, [h.associate, 'Associate', 'associate']);
  const idxLastStatus = idxAny_(headerIndexRaw, [h.lastStatus, 'Last Status', 'last_status']);
  const idxSubDate = idxAny_(headerIndexRaw, [h.claimSubmissionDate, 'Claim Submission Date', 'Submission Date', 'claim_submission_date', 'submission_date']);
  ...
  const idxClaimSubmittedRaw = idxAny_(headerIndexRaw, [h.claimSubmittedDatetime, 'Claim Submitted Datetime', 'Submitted Datetime', 'claim_submitted_datetime', 'submitted_datetime']);

  const idxLastUpdate = idxAny_(headerIndexRaw, [h.lastUpdate, 'Last Update', 'last_update']);
  ...
  const idxPolicyStart = idxAny_(headerIndexRaw, [h.policyStartDate, 'Policy Start Date', 'policy_start_date']);
  const idxPolicyEnd = idxAny_(headerIndexRaw, [h.policyEndDate, 'Policy End Date', 'policy_end_date']);
...
function backupOpsToRawFull_(ss, rawSheet, rawValues, headerIndexRaw, pic) {
  ...
  if (!rawValues || rawValues.length !== rawRowCount) {
    if (rawRowCount === 0) return { updated: 0, notes: 'Raw has no rows' };
    rawValues = rawSheet.getRange(2, 1, rawRowCount, rawSheet.getLastColumn()).getValues();
  }
  if (!rawValues.length) return { updated: 0, notes: 'Raw has no rows' };
